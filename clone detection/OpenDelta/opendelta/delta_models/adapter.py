...
class AdapterLayer(nn.Module, InterFaceMixin):
    ...
    def __init__(self, bottleneck_dim=[24,32], non_linearity='gelu_new', device=None, backend="hf"):
        super().__init__()
    ...
    def instantiate(self, hiddens):
        ...
        self.modulelist.add_module("down_proj",nn.Linear(self.hidden_dim, self.bottleneck_dim[0], device=self.init_device, dtype=self.hidden_dtype))
...
class AdapterConfig(BaseDeltaConfig):
    ...
    def __init__(
        self,
        bottleneck_dim: Optional[List[int]]=[24,32],
